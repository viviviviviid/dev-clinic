...
// A denied file operation is local to that request, not a lost login session.
export function isClinicAuthFailure(status: number, message: string, code?: string): boolean {
  return status === 401 || (status === 403 && (
    code === 'account_not_allowed' || code === 'origin_not_allowed' || code === 'invalid_host'
    || message === 'origin not allowed' || message === 'invalid host'
  ))
}
...
export function describeClinicFailure(failure: ClinicFailure) {
  ...
  return {
    ...
    guidance: failure.kind === 'connection'
      ...
      : failure.status === 403 && (failure.code === 'origin_not_allowed' || failure.message === 'origin not allowed')
